...
(** Return the value associated to a variable in the output of [ocamlc
   -config] *)
...
module C_define : sig
  ...
  (** Import some #define from the given header files. For instance:
      ...
      {[
        # Configurator.C_define.import c ~includes:"caml/config.h"
  ...
  *)
  ...
  (** Generate a C header file containing the following #define.
      ...
      If not specified, it is inferred from the name given to [create]
      and the filename. *)
  val gen_header_file
    :  t
    -> fname:string
